...
def write_to_excel_file(track: str,
    ...
    All Files are written to the '/test_output' directory"""
    ...
    laps_df = load_all_calculated_lap_dataframes(track, telemetry_files)

    if to_csv:
        laps_df.to_csv(f"test_output/{track}.csv")

    with pd.ExcelWriter(f"test_output/{track}.xlsx", engine="openpyxl") as writer:
        try:
            laps_df.to_excel(writer, sheet_name=final_sheet, index=False)
...
if __name__ == "__main__":
    write_to_excel_file("donnington")
